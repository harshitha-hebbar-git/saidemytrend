...
pipeline {                                                              // 1 // Defines the start of the Jenkins pipeline block
	...
	stage("Jar Publish") {                				// 11 // Creates a stage named 'Jar Publish'
            steps {                           				// 12 // Defines the steps that will be executed in this stage
                script {                      				// 13 // Allows running custom Groovy script inside the pipeline
                    ...
                    def buildInfo = server.upload(uploadSpec)		// Uploads the files to Artifactory and collects build info
                    buildInfo.env.collect()  				// Collects environment variables as part of the build info
                    server.publishBuildInfo(buildInfo)			// Publishes the build information to Artifactory
                    echo '<--------------- Jar Publish Ended --------------->'  
                                              				// Logs a message indicating the end of JAR publishing
                }                             				// 13 // Ends the script block for 'Jar Publish' stage
            }                                 				// 12 // Ends the steps block for 'Jar Publish' stage
        }                                     				// 11 // Ends the 'Jar Publish' stage
